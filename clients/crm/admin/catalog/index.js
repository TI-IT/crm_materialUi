...
const AdminCatalog = ({ server_host }) => {
    ...
    async function getAllProductsData(text) {
        fetch(server_host + '/adminCatalogs/getAllData', {
            method: 'get',
            credentials: 'include'
        })
            .then((res) => {
    ...
    const header = (
        <div className="grid">
            ...
            <Button label="Создать" className="bg-green-400 border-white-alpha-10" type="button" onClick={() => router.push('/admin/products/addProducts/')} />
        </div>
    );
